...
const VerificationQueue = ({ queue = [], onApprove, onReject }) => {
  return (
        ...
        <tbody>
          {queue.map((item) => (
            <tr key={item.uid || item.id}>
              <td>{item.name}</td>
              <td><span className={`role-badge ${item.role?.toLowerCase() || 'teacher'}`}>{item.role || 'Teacher'}</span></td>
              <td>
              ...
              <td><span className="status-badge pending">{item.status || 'Pending'}</span></td>
              <td>
